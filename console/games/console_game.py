...
class ConsoleGame:
    ...
    def initialize(self):
        ConsoleGame.print_colored_output("### WELCOME TO QUORIDOR ###", Color.CYAN)
        print("\n")
        ...
        if a == "Y" or a == "y":
            ...
            while True:
                ...
                if not x.isdigit() and x != "x" and x != "X":
                    ConsoleGame.print_colored_output("Illegal input!", Color.RED)
                elif x == "x" or x == "X":
                    exit(0)
                else:
                    if 0 <= int(x) - 1 < len(self.algorithms):
                        self.player_simulation_algorithms[1] = self.algorithms[int(x) - 1]
                        ConsoleGame.print_colored_output("Chosen algorithm for player 2 is {0:30}".format(
            ...
            while True:
                ...
                if not len(x.split(",")) == 2 and x != "x" and x != "X":
                    ...
                    if 0 <= int(one) - 1 < len(self.algorithms) and 0 <= int(two) - 1 < len(self.algorithms):
                        ...
                        ConsoleGame.print_colored_output("Chosen algorithm for player 1 is {0:30}".format(
            ...
            if value == "x" or value == "X":
                ...
                if value.upper().startswith("M"):
                    ...
                    if x_string.upper() not in Mappings.INPUT_MAPPINGS.keys() or y_string.upper() not in Mappings.INPUT_MAPPINGS.keys():
                        ConsoleGame.print_colored_output("Illegal move!", Color.RED)
                    else:
                    ...
                    if x_string.upper() not in Mappings.INPUT_MAPPINGS.keys() or y_string.upper() not in Mappings.INPUT_MAPPINGS.keys():
                        ConsoleGame.print_colored_output("Illegal wall placement!", Color.RED)
                    else:
                        dir_string = value[-1]
                    ...
                    ConsoleGame.print_colored_output("Illegal command!", Color.RED)
